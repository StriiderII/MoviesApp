import 'dart:convert';
import 'package:movies_app/data/Model/Category.dart';
...
class ApiManager {
  ...
  static Future<MoviesModel> getNewReleasesMovies() async {
    try {
      var response = await http.get(Uri.parse(
          '$baseUrl/3/movie/upcoming?api_key=$apiKey&language=en-US'));
      var json = jsonDecode(response.body);
  ...
  static Future<Category> getCategoryMovies() async {
    var response = await http.get(Uri.parse(
        '$baseUrl/3/genre/movie/list?api_key=$apiKey&language=en-US&page=1'));
    var json = jsonDecode(response.body);
  ...
  static Future<MoviesModel> getSearchMovies(String? query) async {
    var response = await http.get(Uri.parse(
        '$baseUrl/3/search/movie?api_key=$apiKey&language=en-US&query=$query&page=1'));
    var json = jsonDecode(response.body);
  ...
  static Future<MoviesModel> getSimilarMovies(num? movieId) async {
    var response = await http.get(Uri.parse(
        '$baseUrl/3/movie/$movieId/similar?api_key=$apiKey&language=en-US&page=1'));
    var json = jsonDecode(response.body);
    MoviesModel categorylist = MoviesModel.fromJson(json);
    return categorylist;
  }
